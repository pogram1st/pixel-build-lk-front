<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
      <div class="p-6">
        <div v-if="!statuses || statuses.length === 0" class="text-center py-8 text-gray-500">
          Нет статусов
        </div>
        <AppTable
          v-else
...
const createStatus = async () => {
  try {
    ...
    toast.showError(error, 'Ошибка создания статуса')
  }
}
...
const deleteStatus = async (id: number) => {
  ...
  try {
    ...
    toast.showError(error, 'Ошибка удаления статуса')
  }
}
</script>
