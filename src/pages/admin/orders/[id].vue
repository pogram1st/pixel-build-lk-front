  ...
  <div v-if="order" class="space-y-6">
    ...
    <div class="flex justify-between items-start">
      ...
      <div class="flex items-center space-x-4">
        <AppButton variant="outline" @click="handleSave">
          Сохранить
        </AppButton>
        <AppButton variant="outline" @click="showStatusModal = true">
          Изменить статус
        </AppButton>
        <AppButton @click="handleSendToClient">
          Отправить клиенту
        </AppButton>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div class="space-y-4">
            ...
            <div v-if="order.files && order.files.filter(f => f.type === 'CLIENT_MATERIAL').length > 0">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Материалы от клиента
              </label>
              <div class="space-y-2">
                <div
                  v-for="file in order.files.filter(f => f.type === 'CLIENT_MATERIAL')"
                  :key="file.id"
            ...
            <div v-if="order.files && order.files.filter(f => f.type === 'ADMIN_FILE').length > 0">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div v-if="order.files && order.files.filter(f => f.type === 'PROGRESS_SCREENSHOT').length > 0" class="space-y-3">
            <div
              v-for="file in order.files.filter(f => f.type === 'PROGRESS_SCREENSHOT').sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())"
              :key="file.id"
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div class="space-y-2">
            <div>
              ...
              <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ order.user?.name }}</span>
            </div>
            <div>
              <span class="text-sm text-gray-500 dark:text-gray-400">Email:</span>
              <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ order.user?.email }}</span>
            </div>
            <div v-if="order.telegram">
              <span class="text-sm text-gray-500 dark:text-gray-400">Telegram:</span>
              <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ order.telegram }}</span>
            </div>
            <div v-if="order.whatsapp">
              <span class="text-sm text-gray-500 dark:text-gray-400">WhatsApp:</span>
              <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ order.whatsapp }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div class="text-sm">
                <span v-if="order.paymentLink" class="text-green-600 dark:text-green-400">Ожидает оплаты</span>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div v-if="order.history && order.history.length > 0" class="space-y-3 max-h-96 overflow-y-auto">
            <div
    ...
    <AppModal v-model="showStatusModal" title="Изменить статус">
      ...
      <template #footer>
        <AppButton variant="outline" @click="showStatusModal = false">
          Отмена
        </AppButton>
    ...
    <AppModal v-model="showUploadModal" title="Загрузить скриншот">
      ...
      <template #footer>
        <AppButton variant="outline" @click="showUploadModal = false">
          Отмена
        </AppButton>
        <AppButton @click="uploadFile" :disabled="!selectedFile">
          Загрузить
        </AppButton>
      </template>
    </AppModal>
...
const handleSave = async () => {
  try {
    ...
    toast.showError(error, 'Ошибка обновления заказа')
  }
}
...
const updateStatus = async () => {
  try {
    ...
    toast.showError(error, 'Ошибка обновления статуса')
  }
}
...
const uploadFile = async () => {
  ...
  try {
    ...
    toast.showError(error, 'Ошибка загрузки файла')
  }
}
...
const deleteFile = async (fileId: number) => {
  ...
  try {
    ...
    toast.showError(error, 'Ошибка удаления файла')
  }
}
