...
const updateStatus = async () => {
  try {
    await adminApi.orders.updateStatus(orderId, { statusId: selectedStatusId.value })
...
const addComment = async () => {
  ...
  try {
    await adminApi.comments.create({ orderId, text: newComment.value })
    toast.success('Комментарий добавлен')
    newComment.value = ''
    await refresh()
  } catch (error: any) {
    toast.showError(error, 'Ошибка добавления комментария')
  }
}
</script>
