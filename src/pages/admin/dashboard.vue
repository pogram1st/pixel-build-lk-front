  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Заказов сегодня</div>
        <div class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
          {{ stats?.ordersToday || 0 }}
        </div>
      </div>
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Заказов за неделю</div>
        <div class="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
          {{ stats?.ordersWeek || 0 }}
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <div class="text-sm font-medium text-gray-500 dark:text-gray-400">Ожидает оплаты</div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
      <div class="p-6 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Последние заказы</h2>
      </div>
      <div class="p-6">
        <div v-if="!stats?.recentOrders || stats.recentOrders.length === 0" class="text-center py-8 text-gray-500">
          Нет заказов
        </div>
          ...
          >
            <div class="flex-1">
              <div class="flex items-center space-x-4">
                ...
                <div v-if="order.service" class="text-sm text-gray-600 dark:text-gray-300">
                  {{ order.service.name }}
                </div>
              </div>
            </div>
