        ...
        >
          <template #cell-role="{ value }">
            <span
              ...
              :class="value === 'ADMIN' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'"
            >
              {{ value }}
            </span>
          </template>
...
const { data: users, pending: loading, refresh } = await useLazyAsyncData('admin-users', async () => {
  const api = await import('~/shared/api/api')
  const { data } = await api.default.get('/admin/users')
  return data
})
...
const createUser = async () => {
  try {
    const api = await import('~/shared/api/api')
    await api.default.post('/admin/users', userForm.value)
...
const updateUser = async () => {
  try {
    const api = await import('~/shared/api/api')
    await api.default.put(`/admin/users/${editForm.value.id}`, editForm.value)
    showEditModal.value = false
    await refresh()
  } catch (error: any) {
    alert(error.response?.data?.message || 'Ошибка обновления пользователя')
  }
}
...
const deleteUser = async (id: number) => {
  ...
  try {
    const api = await import('~/shared/api/api')
    await api.default.delete(`/admin/users/${id}`)
    await refresh()
  } catch (error: any) {
