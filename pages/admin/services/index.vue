<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">
        Управление услугами
      </h1>
      <AppButton @click="showCreateModal = true">
        Создать услугу
      </AppButton>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
      <div class="p-6">
        <div v-if="loading" class="text-center py-8">
          Загрузка...
        </div>
        <AppTable
          v-else
          :columns="columns"
          :data="services"
        >
          <template #cell-price="{ value }">
            {{ value ? `${value} ₽` : '-' }}
          </template>
          <template #cell-actions="{ row }">
            <div class="flex space-x-2">
              <button
                @click="editService(row)"
                class="text-primary-600 hover:text-primary-800"
              >
                Редактировать
              </button>
              <button
                @click="deleteService(row.id)"
                class="text-red-600 hover:text-red-800"
              >
                Удалить
              </button>
            </div>
          </template>
        </AppTable>
      </div>
    </div>

    <AppModal v-model="showCreateModal" title="Создать услугу">
      <div class="space-y-4">
        <AppInput
          v-model="serviceForm.name"
          label="Название"
          placeholder="Название услуги"
        />
        <AppInput
          v-model="serviceForm.price"
          type="number"
          label="Цена"
          placeholder="0"
        />
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Описание
          </label>
          <textarea
            v-model="serviceForm.desc"
            class="w-full px-3 py-2 border border-gray-300 rounded-md"
            rows="3"
            placeholder="Описание услуги"
          />
        </div>
      </div>
      <template #footer>
        <AppButton variant="outline" @click="showCreateModal = false">
          Отмена
        </AppButton>
        <AppButton @click="createService">
          Создать
        </AppButton>
      </template>
    </AppModal>

    <AppModal v-model="showEditModal" title="Редактировать услугу">
      <div class="space-y-4">
        <AppInput
          v-model="editForm.name"
          label="Название"
          placeholder="Название услуги"
        />
        <AppInput
          v-model="editForm.price"
          type="number"
          label="Цена"
          placeholder="0"
        />
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Описание
          </label>
          <textarea
            v-model="editForm.desc"
            class="w-full px-3 py-2 border border-gray-300 rounded-md"
            rows="3"
            placeholder="Описание услуги"
          />
        </div>
      </div>
      <template #footer>
        <AppButton variant="outline" @click="showEditModal = false">
          Отмена
        </AppButton>
        <AppButton @click="updateService">
          Сохранить
        </AppButton>
      </template>
    </AppModal>
  </div>
</template>

<script setup lang="ts">
definePageMeta({
  layout: 'admin',
  middleware: 'admin'
})

const { data: services, pending: loading, refresh } = await useLazyAsyncData('admin-services', async () => {
  const api = await import('~/shared/api/api')
  const { data } = await api.default.get('/admin/services')
  return data
})

const columns = [
  { key: 'id', label: 'ID' },
  { key: 'name', label: 'Название' },
  { key: 'price', label: 'Цена' },
  { key: 'desc', label: 'Описание' },
  { key: 'actions', label: 'Действия' }
]

const showCreateModal = ref(false)
const showEditModal = ref(false)
const serviceForm = ref({
  name: '',
  price: '',
  desc: ''
})
const editForm = ref({
  id: 0,
  name: '',
  price: '',
  desc: ''
})

const createService = async () => {
  try {
    const api = await import('~/shared/api/api')
    await api.default.post('/admin/services', {
      ...serviceForm.value,
      price: serviceForm.value.price ? Number(serviceForm.value.price) : undefined
    })
    showCreateModal.value = false
    serviceForm.value = { name: '', price: '', desc: '' }
    await refresh()
  } catch (error: any) {
    alert(error.response?.data?.message || 'Ошибка создания услуги')
  }
}

const editService = (service: any) => {
  editForm.value = { ...service, price: service.price?.toString() || '' }
  showEditModal.value = true
}

const updateService = async () => {
  try {
    const api = await import('~/shared/api/api')
    await api.default.put(`/admin/services/${editForm.value.id}`, {
      ...editForm.value,
      price: editForm.value.price ? Number(editForm.value.price) : undefined
    })
    showEditModal.value = false
    await refresh()
  } catch (error: any) {
    alert(error.response?.data?.message || 'Ошибка обновления услуги')
  }
}

const deleteService = async (id: number) => {
  if (!confirm('Вы уверены, что хотите удалить эту услугу?')) return
  try {
    const api = await import('~/shared/api/api')
    await api.default.delete(`/admin/services/${id}`)
    await refresh()
  } catch (error: any) {
    alert(error.response?.data?.message || 'Ошибка удаления услуги')
  }
}
</script>

