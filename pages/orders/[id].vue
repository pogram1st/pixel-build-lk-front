        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <span class="ml-2 font-medium">{{ new Date(order.updatedAt).toLocaleString('ru-RU') }}</span>
            </div>
          </div>
        </div>
...
const { data: order, pending: loading } = await useLazyAsyncData(`order-${orderId}`, async () => {
  const api = await import('~/shared/api/api')
  const { data } = await api.default.get(`/orders/${orderId}`)
  return data
})
</script>
