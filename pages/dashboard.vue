...
const { data: orders, pending: loading } = await useLazyAsyncData('orders', async () => {
...
const activeOrders = computed(() => {
  return orders.value?.filter((o: any) => o.status?.name !== 'Завершен').length || 0
})
...
const lastUpdate = computed(() => {
  ...
  const latest = orders.value.reduce((latest: any, order: any) => {
